...
void PS2ControllerNode::read_loop()
{
    ...
    const double max_lateral_speed = 1.0;  // 최대 좌우 이동 속도 (m/s)
    ...
    while(rclcpp::ok())
    {
        ...
        // 아날로그 스틱 값 읽기
        int rx = map_.rx;  // 회전 축
        int ly = map_.ly;  // 전후 이동 축
        int lx = map_.lx;  // 좌우 이동 축

        // 로그 출력 (디버깅 용도)
        RCLCPP_INFO(this->get_logger(), "Raw axis values: rx=%d, ly=%d, lx=%d", rx, ly, lx);
        ...
        normalized_lx = -normalized_lx; // 오른쪽으로 움직이면 양수

        // 속도 계산
        double linear_x = normalized_ly * max_linear_speed;
        double linear_y = normalized_lx * max_lateral_speed;
        ...
        // 로그 출력
        RCLCPP_INFO(this->get_logger(), "Published Twist message: linear.x=%.2f, linear.y=%.2f, angular.z=%.2f", msg.linear.x, msg.linear.y, msg.angular.z);
